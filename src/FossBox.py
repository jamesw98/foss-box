...
class FossBox:
    ...
    def clear_lights(self):
        self.disp.fill_rect(0, 0, self.width, self.forth, self.BLACK)
        self.disp.update()

    def clear_clock(self):
        tw = self.disp.measure_text(self.clock, 1)
        x = (self.width - tw) // 2
        self.disp.fill_rect(x, self.forth, tw, self.eighth, self.BLACK)
        self.disp.update()
        ...
        while True:
            ...
            if any_valid:
                ...
                if right_valid or double:
                    self.disp.fill_rect(self.half, 0, self.half, self.forth, self.RIGHT_COLOR)
                    self.right_score += 1
            ...
            if self.CLOCK_ENABLED and Utils.ticks_diff(Utils.ticks_ms(), self.last_tick) >= 1000 and self.clock_seconds > 0:
                ...
                self.disp.draw_text(self.clock, clock_x, self.forth, self.TIMER_COLOR)

            if self.SCORE_ENABLED:
                left_num = str(self.left_score)
                right_num = str(self.right_score)
                right_x = self.width - self.disp.measure_text(right_num, 1) - self.SCORE_SIDE_PADDING
